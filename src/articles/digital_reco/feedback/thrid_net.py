...
from perceptron import PerceptronR0to1, PerceptronN0to1, Perceptron
...
#        feedback = [PerceptronN0to1(12, 0.1, 0.5, True) for _ in range(10)]
        feedback = [PerceptronR0to1(12, 0.5, 0.5, 1., Perceptron.OUTPUT, True, True) for _ in range(10)]
        
        first_order.init_weights_randomly(-1, 1)
#        high_order_h.init_weights_randomly(-1, 1)
#        for j in range(10):
#            feedback[j].init_weights_randomly(-1, 1)
    ...
    for epoch in range(nbEpoch):
        ...
        for network in networks:
            ...
            for ex in l_exx[0:nbTry]:
                ...
                res = [ network['feedback'][i].calc_output(network['first_order'].stateOutputNeurons + 
                                                           network['high_order_h'].stateOutputNeurons) for i in range(10)]
                ...
                #learn
                for i in range(10):
                    network['feedback'][i].train(network['first_order'].stateOutputNeurons + 
                                                 network['high_order_h'].stateOutputNeurons
    ...
    plt.plot(display_interval , y_perfo['feedback'][3::5], label="feeback", linewidth=2)
