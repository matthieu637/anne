...
from articles.wagering.feedback.merging import ampli
...
if __name__ == '__main__':
    ...
    for i in range(nbr_network):
        ...
        high_order_h2 = MultilayerPerceptron(30, 20, 2, learning_rate=0.1, momentum=0., grid=mode)
        feedback2 = [PerceptronR0to1(30 + 4, 0.1, 0., True) for _ in range(10)]

        networks[i] = {'first_order' : first_order,
                       'first_order2' : first_order2,
                    'high_order_h' : high_order_h,
                    'feedback': feedback,
                    'high_order_h2' : high_order_h2,
                    'feedback2': feedback2}
    ...
    for epoch in range(nbEpoch):
        ...
        for network in networks:
            ...
            for ex in l_exx[0:nbTry]:
                ...
                network['high_order_h2'].calc_output(network['first_order2'].stateHiddenNeurons)
                ...
                res2 = [ network['feedback2'][i].calc_output(network['first_order2'].stateHiddenNeurons + 
                                                           ampli(network['high_order_h2'].stateOutputNeurons, 4)) for i in range(10)]
                if(index_max(res2) == index_max(examples.outputs[ex])):
                    perfo['feedback2'] += 1
                ...
                for i in range(10):
                    network['feedback2'][i].train(network['first_order2'].stateHiddenNeurons + 
                                                 ampli(network['high_order_h2'].stateOutputNeurons, 4)
                                                 , examples.outputs[ex][i])
                
                network['high_order_h'].train(network['first_order'].stateHiddenNeurons,
                                               cell)
                network['high_order_h2'].train(network['first_order2'].stateHiddenNeurons,
                                               cell2)
    ...
    plt.plot(display_interval , y_perfo['feedback'][3::5], label="feedback (on 30)", linewidth=2)
