...
#    y[2] = map(lambda x : x / max_err3, y[2])
    ...
    plt.plot(range(200)[6::5] , y[0][6::5], label="first-order network")
    plt.plot(range(200)[6::5] ,y[1][6::5], label="high-order network (high learning rate)")
#    plt.plot(range(200)[1::5] ,y[2][1::5], label="high-order network (low learning rate)")
    plt.ylabel('ERROR')
    plt.xlabel("EPOCHS")
