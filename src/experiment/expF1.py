...
#   Network Declaration
    ...
    def feedback(inputs, outputs):
        return [PerceptronR0to1(12, 0.1, 0., True) for _ in range(10)]
...
#   Work on one step
    ...
    def step_statictics(simu, network, plot, inputs, outputs):
        res = [ network['feedback'][i].calc_output(network['FoN'].stateOutputNeurons + 
                                           network['SoN'].stateOutputNeurons) for i in range(10)]
    ...
    def step_learn(network, inputs, outputs):
        for i in range(10):
            network['feedback'][i].train(network['FoN'].stateOutputNeurons + 
                                         network['SoN'].stateOutputNeurons
                                         , outputs[i])
    ...
    np.savetxt("expF1.data", sim.plots['feedback'])
