...
#   Network Declaration
    ...
    def feedback(inputs, outputs):
        return [PerceptronR0to1(nbHidden + 10, 0.1, 0., True) for _ in range(10)]
...
#   Work on one step
    ...
    def step_statictics(simu, network, plot, inputs, outputs):
        res = [ network['feedback'][i].calc_output(network['FoN'].stateHiddenNeurons + 
                                           network['FoN'].stateOutputNeurons) for i in range(10)]
    ...
    np.savetxt("expF5.data", sim.plots['feedback'])
